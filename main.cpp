...
int main(int argc, char *argv[]) {
  ...
  if(argc == 1){
    ...
    create(".marko.dat", "dir", get_current_dir_name());
    ...
    if(argc > 2){
      ...
      create(".marko.dat", "def_file", argv[2]);
    ...
    if(exist(".marko.dat")){
      def_file = get_value(".marko.dat", "def_file");
      if(!exist(def_file) || def_file == ""){
        error("default file not set!!");
        return 1;
      }
...
void msg(std::string text){
  std::cout << "\033[32mmarko: \033[0m" << text << std::endl;
}
...
int bootstrap(){
  ...
  else if(get_value(".marko.dat", "dir") != get_current_dir_name()){
      error("please initialise the directory first!!\nprovided .marko.dat isn't real!!");
      return 1;
  }

  else if(get_value(".marko.dat", "def_file") != ""){
    def_file = get_value(".marko.dat", "def_file");
    return 0;
  }
  return 0;
